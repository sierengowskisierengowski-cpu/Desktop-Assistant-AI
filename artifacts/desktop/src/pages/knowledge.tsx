...
import { Plus, Pin, Trash2, Download, Upload, BookOpen, Loader2, PinOff } from "lucide-react";
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        editing ? "border-primary/30 shadow-[0_0_20px_rgba(0,190,210,0.05)]" : "border-white/10 hover:border-white/20",
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          ...
          >
            {note.pinned ? <PinOff className="w-3 h-3 text-primary" /> : <Pin className="w-3 h-3 text-muted-foreground" />}
          </button>
          <button
      ...
      {editing ? (
        ...
        <p className="text-xs text-muted-foreground line-clamp-4 whitespace-pre-wrap">{note.content || "Click to add content..."}</p>
      )}
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        {note.pinned && (
          <Badge className="text-[10px] h-4 px-1.5 bg-primary/10 text-primary border-primary/20">Pinned</Badge>
        )}
      </div>
    </motion.div>
  );
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full min-h-[300px] gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1 max-w-xs">Add notes about your system, preferences, and file structure. AXIOM reads them on every request.</p>
            </div>
            <Button
              size="sm"
